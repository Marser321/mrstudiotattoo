...
INPUT_VIDEO="${INPUT_VIDEO:-/Users/mariomorera/Downloads/Timeline 1.mov}"
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/public/assets/hero-sequence-v2}"
...
main() {
  ...
  echo "Extracting $FRAME_COUNT frames from:"
  echo "  $INPUT_VIDEO"
  ...
  if [[ "$generated_count" != "$FRAME_COUNT" ]]; then
    echo "Error: expected $FRAME_COUNT frames, generated $generated_count" >&2
    exit 1
  fi
