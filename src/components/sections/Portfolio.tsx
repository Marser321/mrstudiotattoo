...
export function Portfolio() {
  ...
  return (
    <section ref={containerRef} className="relative w-full py-20 md:py-24 px-4 md:px-16 lg:px-32 bg-transparent overflow-hidden" id="portfolio">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 auto-rows-[200px] md:auto-rows-[300px]">
        {artworks.map((art, idx) => (
          <Link 
            to="/booking"
            key={art.id} 
            ref={el => { itemsRef.current[idx] = el }}
            className={`block relative group rounded-3xl overflow-hidden luxury-glass border border-border/50 ${art.span} cursor-pointer`}
          >
            <img 
              src={art.img} 
              alt={`Trabajo de portafolio ${idx + 1}`} 
              className="absolute inset-0 w-full h-full object-cover grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-110 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]"
              loading="lazy"
            />
      ...
      <div className="max-w-7xl mx-auto mt-32 flex flex-col md:flex-row items-center gap-16">
        ...
        <div className="w-full md:w-1/2">
          <div className="rounded-3xl overflow-hidden luxury-glass shadow-2xl relative aspect-[4/3] group">
            <img src="/assets/images/tattoo_artist_1776269987044.png" alt="Tattoo artist working" className="w-full h-full object-cover grayscale opacity-90 transition-all duration-1000 group-hover:scale-105 group-hover:grayscale-0" loading="lazy" />
