...
function resolveHeaderHeroConfig(input?: HeaderHeroConfigInput): HeaderHeroConfig {
  return {
    ...headerHeroConfig,
    ...input,
    cards: input?.cards?.length ? input.cards : headerHeroConfig.cards,
    fonts: {
      ...headerHeroConfig.fonts,
      ...input?.fonts,
    },
    visualFraming: {
...
export function HeroSequenceHeader({ config }: HeroSequenceHeaderProps) {
      ...
      <div ref={stageRef} className={cn("relative overflow-hidden", isMobileViewport ? "min-h-screen" : "h-screen")}>
        <div className="relative h-full w-full" style={shellStyle}>
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-black/58 via-black/12 to-black/50" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#04182f] via-transparent to-black/20" />
            ...
            <div className="absolute bottom-4 left-4 z-20 max-w-[80%] md:bottom-9 md:left-8 md:max-w-[26rem]">
              ...
              <a
                href={resolvedConfig.ctaHref}
                className={cn(
                  resolvedConfig.fonts.buttonClass,
                  "inline-flex items-center justify-center border border-white/60 bg-white/6 px-6 py-3 text-xs uppercase tracking-[0.22em] text-white transition-colors duration-300 hover:bg-white hover:text-black",
                )}
              >
              ...
              >
                {resolvedConfig.cards.map((card) => (
                  <article
                    ...
                    data-id={card.id}
                    className={cn(
                      "flex min-h-full flex-col justify-end gap-3 border border-white/14 bg-gradient-to-b from-white/7 to-white/[0.02] p-4 shadow-[0_20px_55px_rgba(0,0,0,0.35)] backdrop-blur-[8px] backdrop-saturate-150 transition-transform duration-300 hover:-translate-y-1 hover:border-white/25",
