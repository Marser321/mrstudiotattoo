...
export function SimpleGallery() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 md:p-8 bg-black">
      {galleryImages.map((src, index) => (
        <div 
          key={index} 
          className="relative aspect-square overflow-hidden border border-white/5 group hover:border-red-600/50 transition-all duration-700"
        >
          <img 
            src={src} 
            alt={`Work ${index + 1}`} 
